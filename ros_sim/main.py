...
class SimCar(Widget):
    ...
    def get_effective_angle(self, relative_angle):
        ...
        relative_angle = 65 if relative_angle > 65 else - \
            65 if relative_angle < -65 else relative_angle
...
class Simulator(Widget):  # Root Widget
    ...
    def trace_route(self, point1, point2, point3, point4):
        ...
        if len(RENDERED_FRAMES) % 10 == 0:
            with self.canvas:
                ...
                self.mark_lines[len(self.mark_lines) -
                                1] = Line(circle=(point1[0], point1[1], 1), width=1)

                Color(0, 1, 0)

                self.mark_lines.append(ObjectProperty(None, allownone=True))
                self.mark_lines[len(self.mark_lines) -
                                1] = Line(circle=(point2[0], point2[1], 1), width=1)

                Color(0, 0, 1)
                self.mark_lines.append(ObjectProperty(None, allownone=True))
                self.mark_lines[len(self.mark_lines) -
                                1] = Line(circle=(point3[0], point3[1], 1), width=1)
                Color(1, 1, 1)

                self.mark_lines.append(ObjectProperty(None, allownone=True))
                self.mark_lines[len(self.mark_lines) -
                                1] = Line(circle=(point4[0], point4[1], 1), width=1)
            ...
            try:
                with open("maps.json", "r") as mapfile:
                    ...
                    for i, line in enumerate(b):
                        ...
                        _b = _line[6]
                        Color(r, g, _b)
                        self.barriers[len(self.barriers) -
                                      i - 1] = Line(points=points)
                    # Draw the target
                    ...
                    if shape_no > 0:
                        self.camera_shapes = [
                            ObjectProperty(
                                None, allownone=True) for e in range(
                                len(b) + 1)]
                        for i, shape in enumerate(shapes):
                            ...
                            _b = _shape[5]
                            Color(r, g, _b)
                            self.camera_shapes[shape_no - i - 1] = Line(
                                circle=(center[0], center[1], radious), width=3)
    ...
    def check_for_target(self):
        ...
        p2 = TARGET_CENTER
        # 60 is an arbitrary radious to determine if the vehicle is close
        # enough
    ...
    def save_simulation(self):
        ...
        if SAVE_TRIGGER == SAVE_OPTIONS.FAILURE \
            ...
            if not SAVE_FILE:
                SAVE_FILE = True
                tag = STATUS_LABEL
                file_name = "simulations/" + tag + \
                    str(datetime.now()) + "||" + str(MAP_SELECTION)
        ...
        for shape in self.camera_shapes:
            ...
            if contains((circlex, circley), cam):
                ...
                for b in self.barriers:
                    ...
                    shot = find_intersection(
                        circle_point, car_point, barrier_point1, barrier_point2)
                ...
                if saw_shape:
                    ...
                    make_image(dist, PHOTO_FRAME_COUNT + 1, x_offset_diff)
                    PHOTO_FRAME_COUNT += 1
    ...
    def update(self, dt):
        ...
        if barrier_collision or border_collision or target_reached:
            ...
            try:
                camera_p2_x = (
                    math.cos(
                        ((self.car.angle + 35) * math.pi) / 180) * CAMERA_RANGE) + car_center_x

                camera_p2_y = (
                    math.sin(
                        ((self.car.angle + 35) * math.pi) / 180) * CAMERA_RANGE) + car_center_y
            ...
            if distance is not 10000:
                ...
                msg.pid_error = interp(
                    distance, [0, meters_to_pixels(2.5)], [-100, 100])
            else:
                msg.pid_error = 10000
